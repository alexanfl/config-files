...
const setContextMenuForSerp = async (request, sender, callback) => {
    ...
    try {
        ...
        await asyncContextMenuCreation(tabId, {
            ...
            id: parentId,
            targetUrlPatterns: ["*://" + request.host + "/*&url=" + encodeURIComponent(request.href) + "&usg=*"],
            contexts: ["link"],
        ...
        if (!request.styles) {
            ...
            await Promise.all(request.styles.map(style => {
                return asyncContextMenuCreation(tabId, {
                    ...
                    parentId: parentId,
                    onclick: () => {
                        analyticsMainEventReport('SERP', 'Theme clicked', style.style_url);
            ...
            if (request.showMore) {
                ...
                await asyncContextMenuCreation(tabId, {
                    ...
                    parentId: parentId,
                    onclick: () => {
                        analyticsMainEventReport('SERP', 'View More clicked');
